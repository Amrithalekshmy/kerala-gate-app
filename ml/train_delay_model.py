"""
ML Model 1 — Train Delay Prediction

Predicts how many minutes late a train will be at a given gate.
Uses Random Forest Regressor trained on historical delay data.

Input features:
  - train_number (encoded)
  - day_of_week (0=Mon, 6=Sun)
  - month (1-12)
  - scheduled_hour (0-23)
  - prev_station_delay (minutes late at last station)
  - distance_from_origin (km)
  - is_express (1 for express/superfast, 0 for passenger/local)

Output: predicted_delay_minutes

Training data source:
  data/historical_delays.csv or data.gov.in / Kaggle datasets

Accuracy target: MAE under 10 minutes for v1, under 5 for v2.
"""
...
def load_training_data():
    ...
    Expected CSV columns: train_number, station_code, scheduled_arrival,
                          actual_arrival, delay_minutes, recorded_date,
                          day_of_week, month
    """
...
def generate_synthetic_data():
    """
    Generate synthetic delay data for demo/testing.
    Based on realistic Indian Railways delay patterns.
    """
    np.random.seed(42)
    n_samples = 10000
    ...
    for _ in range(n_samples):
        ...
        # Express trains (1xxxx) vs passenger (5xxxx/6xxxx)
        is_express = 1 if train_num.startswith('1') or train_num.startswith('2') else 0

        # Base delay: express trains slightly more punctual
        base_delay = np.random.exponential(8 if is_express else 15)

        # Monsoon effect (June-September): more delays
        if 6 <= month <= 9:
            base_delay *= 1.5

        # Rush hour effect (7-10am, 5-8pm)
        if 7 <= scheduled_hour <= 10 or 17 <= scheduled_hour <= 20:
            base_delay *= 1.3

        # Weekend: slightly fewer delays
        if day_of_week >= 5:
            base_delay *= 0.85

        # Distance effect: delays compound over longer distances
        base_delay += distance * 0.005

        # Previous station delay (correlated with actual delay)
...
def train_model(df):
    """Train the Random Forest delay prediction model."""
    print(f"Training on {len(df)} records...")

    # Encode train numbers
